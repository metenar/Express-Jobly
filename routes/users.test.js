...
describe("POST /users/:username/jobs/:id", function () {
  test("works for users or admin", async function () {
    const resp = await request(app)
  ...
  test("not found if user is invalid", async function () {
    const resp = await request(app)
        .post(`/users/nope/jobs/1`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(404);
  });
  test("not found if job is invalid", async function () {
    const resp = await request(app)
        .delete(`/users/c1/jobs/999999`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(404);
  });
});